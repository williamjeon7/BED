...
float servo_microseconds, servo_answer;
...
// get a servo_microseconds from servo equation
  servo_microseconds = Servo_equation(dist_ema);
...
// adjust servo position according to the USS read value
  ...
  if(dist_ema < 180){
     ...
     myservo.writeMicroseconds(servo_microseconds);
...
float Servo_equation(float dist_ema)
{ 
  servo_answer = 553+(923.0/90.0)*(dist_ema-180.0);
  return servo_answer;
}
