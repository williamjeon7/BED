...
// get a brightness level from the equation
 led_brightness = Brightness_equation(dist_raw,dist_max,dist_min);
...
float Brightness_equation(float dist_raw, float dist_max, float dist_min)
{
  if(dist_raw < (dist_min+dist_max)/2){
    answer = ((-2.55)*(dist_raw) + 510);
    return answer;
  }
  else{
    answer = ((2.55)*(dist_raw) - 510);
